...
namespace Background
{
	...
	class AppContext : ApplicationContext
	{
		...
		private static readonly int IMAGE_LEVEL = 8;
		private static readonly string BASE_URL = @"https://himawari8-dl.nict.go.jp/himawari8/img/D531106/";
		...
		private void Update(object arg)
		{
			...
			try
			{
				string url = getUrl();
				img = DownloadImg(url);
				if (!ImgIsNotEmpty(img))
					throw new Exception("The invalid image (\"No Image\") was received");
				setWallpaper(img);
			}
			catch (Exception e)
			{
			...
			if (success)
			{
				...
				notify.SetNotifyIconText(Status.Message
					+ "\n Next try: " +
					Status.LastAttempt.AddMinutes(10).ToString("HH:mm::ss"));
			if (img != null) img.Dispose();
			GC.Collect();
		}

		private bool ImgIsNotEmpty(Image img)
		{
			var bitMap = new Bitmap(img);

			int horisontalCentre = bitMap.Width / 2;
			int vertiacalCentre = bitMap.Height / 2;

			for (int i = 0; i < bitMap.Width; i++)
			{
				if ((bitMap.GetPixel(i, vertiacalCentre).ToArgb() & 0x0000FFFFFFFFFFFF) != 0)
					return true;
			}
		...
		string getUrl()
		{
			...
			var dtTarget = dtNow
				.AddMinutes(-dtNow.Minute % 10)  // Round minutes to 10
				.AddMinutes(-10)                 // Some buffer, won't work without it
				.AddSeconds(-dtNow.Second);      // Remove seconds

			var blackOutTime = new List<string>() { "23:00", "23:10" };
			var himawariTimeStr = DateTime.Now.ToUniversalTime().AddHours(9).ToString(@"HH:mm").Substring(0, 4) + "0";
			if (blackOutTime.Contains(himawariTimeStr))
				throw new Exception("The current image is not available due to the satellite position.");

			return BASE_URL + IMAGE_LEVEL + "d/" + BLOCK_WIDTH + '/' +
				dtTarget.ToString(@"yyyy\/MM\/dd\/HHmmss");
		}
